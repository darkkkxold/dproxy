...
class ThreeProxyManager:
    ...
    def generate_config(self, proxy_instances, backconnect_ip: str) -> str:
        """
        Генерирует конфигурацию 3proxy с правильным порядком директив
        """
        config_lines = [
            "# Основные настройки",
            "daemon",
            "nserver 1.1.1.1",  # Primary DNS
            "nserver 8.8.8.8",  # Fallback DNS
            "nscache 65536",  # DNS cache
            "timeouts 1 5 30 60 180 1800 15 60",  # Connection timeouts
            f"maxconn {len(proxy_instances) * 10}",  # Connection limit
            ""
        ]

        # Настройки сети для IPv6 (добавлены 2 важные директивы)
        config_lines.extend([
            "# Сетевые настройки для IPv6",
            f"internal {backconnect_ip}",
            "external ::",  # Разрешить все IPv6-адреса
            "ipv6",  # Явное включение IPv6 поддержки
            "socks -p0",  # Блокировка стандартного SOCKS-порта
            "proxy -p0",  # Блокировка стандартного HTTP-порта
            ""
        ])
